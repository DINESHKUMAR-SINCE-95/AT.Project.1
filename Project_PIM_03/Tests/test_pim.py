...
from Webpage.login_page import PIMPage
...
def test_delete_existing_employee(setup):
    driver = setup
    pim_page = PIMPage(driver)
    ...
    # Search for an existing employee
    pim_page.search_employee("John Doe")
    pim_page.select_employee_checkbox()
    ...
    # Assert success message
    expected_success_message = "Successfully Deleted"  # Adjust the expected message based on the actual application
    assert expected_success_message in pim_page.get_success_message()